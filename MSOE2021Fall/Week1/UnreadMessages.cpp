...
int main() {
    ...
    int n, m;
    cin >> n >> m;
    segment_tree<long long> seg_tree(n);
    for (int i = 0; i < m; ++i) {
        int person;
        cin >> person;
        --person;
        seg_tree.add_range(0, n - 1, 1);
//        seg_tree.dfs();

        int curr_msgs = seg_tree.sum(person, person);

        seg_tree.add(person, -curr_msgs);

        cout << seg_tree.sum(0, n - 1) << endl;
    }
