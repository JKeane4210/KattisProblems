...
int minimalGame(bitset<49> currGrid) {
    int res = -1;
    if (gridResults.find(currGrid) != gridResults.end()) {
        return gridResults[currGrid];
    }
    ...
    for (auto move: validMoves(currGrid)) {
        int moveRes = minimalGame(makeMove(currGrid, move));
        if (moveRes >= 0) {
            int possRes = 1 + moveRes;
            if (res == -1) {
                res = INT_MAX;
            }
            res = min(res, possRes);
    ...
    if (res >= 0) {
        memoizeAllConfigs(currGrid, res);
    } else if (res == -1) {
