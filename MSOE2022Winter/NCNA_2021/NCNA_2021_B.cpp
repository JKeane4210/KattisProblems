...
int highestOneBit(int i) {
    ...
    i |= (i >>  8);
    i |= (i >> 16);
    return (unsigned int)i - (((unsigned int)i) >> 1);
}
...
int main() {
    ...
    for (int j = 0; j < max_pow; ++j) {
        for (int i = 1; i <= n; ++i) {
            if (j == 0) {
                k_parent[i][j] = parents[i];
            } else {
                if (depths[i] < (1 << j)) continue;
                k_parent[i][j] = k_parent[k_parent[i][j - 1]][j - 1];
            }
        }
    }
...
//    cout << endl;
    ...
    for (int j = 0; j < max_pow; ++j) {
        for (int i = 1; i <= n; ++i) {
            if (j == 0) {
                forward[i][j] = digits[i] % m;
                backward[i][j] = digits[i] % m;
            } else {
                if (depths[i] < (1 << j)) continue;
                forward[i][j] = (base_ten_mod[1 << (j - 1)] * forward[i][j - 1]) % m;
                forward[i][j] = (forward[i][j] + forward[k_parent[i][j - 1]][j - 1]) % m;
                backward[i][j] = (base_ten_mod[1 << (j - 1)] * backward[k_parent[i][j - 1]][j - 1]) % m;
                backward[i][j] = (backward[i][j] + backward[i][j - 1]) % m;
            }
        }
    }
