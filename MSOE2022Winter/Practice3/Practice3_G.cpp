...
int main() {
    ...
    vector<segment_tree<long>> rows;
    vector<segment_tree<long>> cols;
    for (int i = 0; i < 4500; ++i) {
        rows.pb(segment_tree<long>(4500));
        cols.pb(segment_tree<long>(4500));
    }
    ...
    for (int i = 0; i < r; ++i) {
        int x, y, w, h, c;
        cin >> x >> y >> w >> h >> c;
        rows[y + 2250].add_range(x + 2250, x + 2250 + w - 1, c);
        rows[y + 2250 + h].add_range(x + 2250, x + 2250 + w - 1, c);
        cols[x + 2250].add_range(y + 2250, y + 2250 + h - 1, c);
        cols[x + 2250 + w].add_range(y + 2250, y + 2250 + h - 1, c);
    }
    ...
    for (int i = 0; i < q; ++i) {
        ...
        cin >> x >> y >> w >> h;
        long long total = 0;
        for (int row = y + 2250; row <= y + 2250 + h; ++row) {
            total += rows[row].sum(x + 2250, x + 2250 + w - 1);
        }
        for (int col = x + 2250; col <= x + 2250 + w; ++col) {
            total += cols[col].sum(y + 2250, y + 2250 + h - 1);
        }
        cout << total << endl;
    }
